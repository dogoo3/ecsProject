...
public partial struct CharacterSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        _sphereEntity = SystemAPI.GetSingletonEntity<CharacterComponent>();
        _inputEntity = SystemAPI.GetSingletonEntity<InputComponent>();

        _characterComponent = entityManager.GetComponentData<CharacterComponent>(_sphereEntity);
    ...
    private void Move(ref SystemState state)
    {
        ...
        sphereTransform.Position += new float3(forward3 * _inputComponent.movement.z * _characterComponent.moveSpeed * Time.deltaTime);
        sphereTransform.Position += new float3(right3 * _inputComponent.movement.x * _characterComponent.moveSpeed * Time.deltaTime);
    ...
    private void CameraRotate(ref SystemState state)
    {
        ...
        foreach(var(character, entity) in SystemAPI.Query<CharacterComponent>().WithEntityAccess())
        {
